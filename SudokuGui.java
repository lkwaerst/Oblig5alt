...
class SudokuGui implements ActionListener {
    ...
    private int teller = 0;         //teller hvilken loesing som vises
    private JLabel tellerVindu;
    
    SudokuGui (int[][] sudoku, int boksHoeyde, int boksLengde, Kontroll k) {
	kontroll = k;
	this.sudoku = sudoku;
	...
	visningsPanel.setBorder(BorderFactory.createLineBorder(Color.black, 1));
	...
	nesteLoesningKnapp = new JButton("Neste");
	...
	vindu.setSize(stoerrelse * 100, stoerrelse * 100);
	vindu.setVisible(true);
	malBokser();

    }

    //viser neste loesning
	...
	if (teller == antLoesninger) {
	    nesteLoesningKnapp.setEnabled(false);
	}
    }
    
    //metode for aa velge fil
    ...
    //metode som tar seg av knappetrykk
